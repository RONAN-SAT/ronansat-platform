...
export default function FullLengthDashboard() {
  ...
  useEffect(() => {
    if (session) {
      const fetchUserStats = async () => {
        ...
        } catch (e) { console.error("Failed to load user stats", e); }
      };
  ...
  if (!session) return null; // Tránh render chớp nhoáng trước khi bị đá về "/"
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-12">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <TestLibrary 
          uniquePeriods={uniquePeriods} selectedPeriod={selectedPeriod} setSelectedPeriod={setSelectedPeriod}
          sortOption={sortOption} setSortOption={setSortOption} page={page} setPage={setPage}
          loading={loading} filteredTests={filteredTests} totalPages={totalPages}
        />
      </main>
    </div>
  );
